...
- `docs/WRF-WINDS.md` — basin-winds-style figures from `wrfout` (`brc_tools/nwp/wrf_section.py` + `visualize/wrf_curtain.py` + `visualize/coldpool3d.py` + `visualize/profile.py` + `scripts/wrf_winds.py --config <case.toml>` + `scripts/wrf_winds.dtn.slurm`); `/wrf-basin-winds` skill. Four families via `--figure`: `topdown` (incl. `tsk_minus_t2`, `snow_depth`, `conv_10m`), `section`, `profile` (θ + humidity + wind panel), `view3d`. Arbitrary A→B transects flat-shaded on **native eta cells**; matched across nests, sweeps a still-writing run. **SSOT for `w_exag` and for section fill/sign conventions** (`speed` = magnitude, `along` = + toward B, `normal` = + into page). Per-case TOMLs live in the repo owning the case.
